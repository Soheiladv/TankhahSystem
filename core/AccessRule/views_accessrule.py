...
class PostAccessRuleAssignView_OLdModel(FormView):
...
class PostAccessRuleAssignView(PermissionBaseView, FormView):
    ...
    def get_form_kwargs(self):
        kwargs = super().get_form_kwargs()
        # posts_query = Post.objects.filter(is_active=True).select_related('organization', 'parent')
        posts_query = Post.objects.filter(is_active=True).select_related('organization', 'parent').order_by('level')  # مرتب‌سازی بر اساس level
