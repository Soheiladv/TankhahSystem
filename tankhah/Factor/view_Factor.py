        ...
        if tankhah:
            try:
                ...
                if not project_allocation:
                    ...
                    tankhah_remaining_budget = project_budget - consumed + returned
                    project_remaining_budget = tankhah_remaining_budget  # فرض بر یکسان بودن بودجه پروژه و تنخواه

                    budget_info = {
                        'project_name': project.name,
                        'project_budget': project_budget,
                        'project_consumed': consumed,
                        'project_returned': returned,
                        'project_remaining': project_remaining_budget,
                        'tankhah_budget': project_budget,
                        'tankhah_consumed': consumed,
                        'tankhah_remaining': tankhah_remaining_budget,
                    }
...
class File_404_02_15_FactorCreateView(PermissionBaseView, CreateView):
    model = Factor
    form_class = FactorForm
    template_name = 'tankhah/factor_form.html'
    success_url = reverse_lazy('factor_list')
