    ...
    def form_valid(self, form):
        ...
        try:
            ...
            if not project_allocation or not project_allocation.budget_allocation:
                raise ProjectBudgetAllocation.DoesNotExist("خطای عدم تخصیص بودجه.")
...
class FactorUpdateView(PermissionBaseView, UpdateView):
    ...
    def form_valid(self, form):
        ...
        # بررسی تخصیص بودجه
        try:
            project_allocation = tankhah.project_budget_allocation or ProjectBudgetAllocation.objects.filter(
                budget_allocation=tankhah.budget_allocation,
                project=tankhah.project,
                subproject=tankhah.subproject
            ).first()
            if not project_allocation or not project_allocation.budget_allocation:
                raise ProjectBudgetAllocation.DoesNotExist
