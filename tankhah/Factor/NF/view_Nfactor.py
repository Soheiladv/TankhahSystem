    ...
    def form_valid(self, form):
        ...
        try:
            ...
            if not tankhah.current_stage or tankhah.current_stage.order != initial_stage_order:
                ...
                msg = _('فقط در مرحله اولیه ({}) می‌توانید فاکتور ثبت کنید. مرحله فعلی تنخواه: {}').format(initial_name,
                                                                                                           stage_name)
        ...
        if not (is_item_formset_valid and is_doc_form_valid and is_tankhah_doc_form_valid):
            ...
            if not is_item_formset_valid: logger.error(
                f"Item formset errors: {item_formset.errors} {item_formset.non_form_errors()}")
            if not is_doc_form_valid: logger.error(f"Factor document form errors: {document_form.errors}")
            if not is_tankhah_doc_form_valid: logger.error(
                f"Tankhah document form errors: {tankhah_document_form.errors}")
            messages.error(self.request, _('لطفاً خطاهای مشخص شده در فرم ردیف‌ها یا اسناد را بررسی و اصلاح کنید.'))
            return self.render_to_response(
                self.get_context_data(form=form, formset=item_formset, document_form=document_form,
        ...
        if abs(total_items_amount - factor_form_amount) > tolerance:
            msg = _('مبلغ کل فاکتور ({}) با مجموع مبلغ ردیف‌ها ({}) همخوانی ندارد.').format(factor_form_amount,
                                                                                            total_items_amount)
            messages.error(self.request, msg)
        ...
        try:
            ...
            if total_items_amount > tankhah_remaining:
                msg = _('مبلغ فاکتور ({:,}) از بودجه باقی‌مانده تنخواه ({:,}) بیشتر است.').format(total_items_amount,
                                                                                                  tankhah_remaining)
                messages.error(self.request, msg)
                logger.error(
        ...
        try:
            project_allocation = ProjectBudgetAllocation.objects.select_related('budget_allocation').filter(
                project=tankhah.project).first()
            if not project_allocation or not project_allocation.budget_allocation:
                raise ProjectBudgetAllocation.DoesNotExist("تخصیص بودجه معتبر یافت نشد.")
        ...
        try:
            ...
            logger.error(
                f"Validation Error during atomic save: {ve.message_dict if hasattr(ve, 'message_dict') else ve}",
                exc_info=True)
