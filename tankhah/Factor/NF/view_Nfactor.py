...
###############################################################################################
class ____New_FactorCreateView(PermissionBaseView, CreateView):
    model = Factor
    form_class = FactorForm
    ...
    def get_form_kwargs(self):
        ...
        kwargs = super().get_form_kwargs()
        kwargs['user'] = self.request.user
        tankhah_id = self.kwargs.get('tankhah_id') or self.request.GET.get('tankhah_id') or self.request.POST.get(
            'tankhah')
    ...
    #     if tankhah_id:
    #         try:
    #             tankhah = Tankhah.objects.select_related('project', 'budget_allocation', 'organization').get(
    #                 id=tankhah_id)
    ...
    #                 }
    #                 context['budget_info'] = budget_info
    #             else:
    #                 logger.error(f"No budget_allocation for tankhah {tankhah.number}")
    #                 context['budget_info'] = {'error': 'No budget allocation for this tankhah'}
    #         except Tankhah.DoesNotExist:
    #             logger.error(f"Tankhah with ID {tankhah_id} not found")
    #             context['budget_info'] = {'error': 'Tankhah not found'}
    #         except Exception as e:
    #             logger.error(f"Error calculating budget info for Tankhah {tankhah.number}: {str(e)}", exc_info=True)
    #             context['budget_info'] = {'error': str(e)}
    #     return context
    ...
    def get_context_data(self, **kwargs):
        ...
        # تعریف فرم‌ست و فرم‌های اسناد
        if self.request.POST:
            context['formset'] = FactorItemFormSet(self.request.POST, self.request.FILES, instance=self.object)
            context['document_form'] = FactorDocumentForm(self.request.POST, self.request.FILES)
            context['tankhah_document_form'] = TankhahDocumentForm(self.request.POST, self.request.FILES)
        else:
            context['formset'] = FactorItemFormSet(instance=self.object)
            context['document_form'] = FactorDocumentForm()
            context['tankhah_document_form'] = TankhahDocumentForm()
        ...
        if tankhah_id:
            try:
                tankhah = Tankhah.objects.select_related('project', 'project_budget_allocation', 'organization').get(
                    id=tankhah_id)
    ...
    def form_invalid(self, form):
        ...
        logger.error(f"Main FactorForm is invalid. Errors: {form.errors.as_data()}")
        ...
        if form.errors or item_formset.errors:
            messages.error(self.request, _('لطفاً خطاهای مشخص شده در فرم را اصلاح کنید.'))
    ...
    def form_valid(self, form):
        ...
        is_draft = 'save_draft_incomplete' in self.request.POST  # اصلاح دکمه ذخیره موقت
        save_final = 'save_final_draft' in self.request.POST or not is_draft
        ...
        try:
            ...
            if not initial_stage:
                logger.error("No WorkflowStage defined.")
                messages.error(self.request, _('هیچ مرحله گردش کاری تعریف نشده است.'))
                return self.form_invalid(form)
        ...
        try:
            ...
            if not budget_allocation_instance or not budget_allocation_instance.is_active:
                logger.error(f"Invalid or inactive BudgetAllocation for tankhah {tankhah.number}")
                messages.error(self.request, _('تخصص بودجه معتبر یا فعال برای این تنخواه یافت نشد.'))
                return self.form_invalid(form)
        except BudgetAllocation.DoesNotExist:
            logger.error(f"No BudgetAllocation found for tankhah {tankhah.number}")
            messages.error(self.request, _('تخصص بودجه معتبر برای این تنخواه یافت نشد.'))
            return self.form_invalid(form)
        ...
        if not (is_item_formset_valid and is_doc_form_valid and is_tankhah_doc_form_valid):
            ...
            messages.error(self.request, _('لطفاً خطاهای مشخص شده در فرم ردیف‌ها یا اسناد را بررسی کنید.'))
            return self.render_to_response(
                self.get_context_data(
                    form=form,
                    formset=item_formset,
        ...
        factor_form_amount = form.cleaned_data.get('amount', Decimal('0')).quantize(Decimal('0.01'))
        ...
        try:
            ...
            logger.debug(f"Tankhah {tankhah.number} remaining budget: {tankhah_remaining}")
            if tankhah_remaining == Decimal('0'):
                logger.warning(f"Zero remaining budget for tankhah {tankhah.number}")
                messages.error(self.request, _('بودجه باقی‌مانده تنخواه صفر است.'))
                return self.form_invalid(form)
        ...
        # چک قفل دوره بودجه
        try:
            # is_locked = budget_allocation_instance.budget_period.is_locked
            # if is_locked:
            #     form.add_error(None, _("امکان ثبت هزینه وجود ندارد: دوره بودجه قفل شده است."))
            #     messages.error(self.request, _("امکان ثبت هزینه وجود ندارد: دوره بودجه قفل شده است."))
            #     logger.error(f"Budget period locked for tankhah {tankhah.number}")
            #     return self.form_invalid(form)
            is_locked, lock_message = budget_allocation_instance.budget_period.is_locked
            if is_locked:
            ...
            logger.error(f"Error checking budget period lock status: {e}", exc_info=True)
            messages.error(self.request, _('خطا در بررسی وضعیت قفل دوره بودجه.'))
            return self.form_invalid(form)
        ...
        try:
            ...
            logger.error(f"Error during transaction: {e}", exc_info=True)
            messages.error(self.request, _('خطا در ذخیره اطلاعات.'))
            return self.form_invalid(form)

        messages.success(self.request, _('فاکتور با موفقیت ذخیره شد.'))
        return super().form_valid(form)
