...
class New_FactorCreateView(PermissionBaseView, CreateView):
    ...
    def form_valid(self, form):
        ...
        context = self.get_context_data() # Get context with initialized forms/formset
        ...
        is_draft = 'save_draft' in self.request.POST  # بررسی دکمه ذخیره موقت
        ...
        save_final = 'save_final_draft' in self.request.POST or not save_incomplete  # Default to final save
        logger.info(f"Form submission type: {'Incomplete Draft' if save_incomplete else 'Final Draft'}")
        # --- بررسی‌های اولیه (برای هر دو نوع ذخیره لازم است) ---


        # --- Initial Tankhah Checks ---
        try:
            ...
            if not tankhah.current_stage or tankhah.current_stage.order != initial_stage_order:
                ...
                # Re-render with the main form error implicitly handled by form_invalid pathway
                return self.form_invalid(form) # Use form_invalid to render context correctly
        except Exception as e:
        ...
        # اگر حتی فرم‌ها پایه معتبر نیستند، برای هیچ نوع ذخیره‌ای ادامه نده
        if not (is_item_formset_valid and is_doc_form_valid and is_tankhah_doc_form_valid):
            ...
            # Re-render the template with the context containing invalid forms
            # Important: Use render_to_response, not form_invalid, because the *main* form is valid here.
        ...
        # Calculate total and check against budget/factor amount
        # Amount was calculated and stored in FactorItemForm.clean
        ...
        if abs(total_items_amount - factor_form_amount) > tolerance:
            ...
            # Add error to the main amount field for user feedback
            form.add_error('amount', msg)
            return self.render_to_response(self.get_context_data(form=form, formset=item_formset, document_form=document_form, tankhah_document_form=tankhah_document_form))

        # Check against Tankhah budget
        try:
            tankhah_remaining = get_tankhah_remaining_budget(tankhah)
            if total_items_amount > tankhah_remaining:
                 ...
                 # Add error to non_field_errors or amount field
        ...
        # Find Budget Allocation for the transaction         # چک کردن تخصیص بودجه پروژه
        try:
            # project_allocation = ProjectBudgetAllocation.objects.select_related('budget_allocation').get(project=tankhah.project)
            ...
            logger.error(f"No valid ProjectBudgetAllocation/BudgetAllocation found for project {tankhah.project.name}")
        ...
        if is_locked:
            ...
            # Rollback might happen automatically due to error, or raise validation error
            raise ValidationError( ("عملیات به دلیل قفل بودن دوره بودجه مجاز نیست."))  # This will trigger rollback in atomic block

        # --- Atomic Save Operation ---     # --- ذخیره اتمی ---
        try:
            with transaction.atomic():
                # 1. Save main factor object             # 1. ذخیره فاکتور اصلی
                self.object = form.save(commit=False)
                # self.object.status = 'DRAFT'
                self.object.status = 'DRAFT' if is_draft else 'PENDING'
                self.object.created_by = self.request.user
                # Set amount definitively from item sum
                # برای ذخیره موقت، شاید نخواهیم amount را ست کنیم یا از factor_form_amount استفاده کنیم
                ...
                for item_form in valid_item_forms:
                    ...
                    item.factor = self.object # Link item to the saved factor
                    # Let FactorItem's save method calculate its own amount
                    item.save()
                    items_saved_count += 1
                logger.info(f"Saved {items_saved_count} factor items for Factor pk={self.object.pk}.")

                # 3. Save factor documents  # 3. ذخیره اسناد فاکتور (برای هر دو نوع ذخیره)
                factor_files = document_form.cleaned_data.get('files', [])
                docs_saved_count = 0
                for file in factor_files:
                    # **مهم:** از فیلد uploaded_by (اگر اضافه کرده‌اید) استفاده کنید
                ...
                # 4. Save tankhah documents (Optional, if needed from this form)   # 4. ذخیره اسناد تنخواه (برای هر دو نوع ذخیره)
                ...
                # 5. Handle document deletion (check POST data)
                #    (Keep the deletion logic from the optimized view)
                ...
                for key in self.request.POST:
                    if key.startswith('delete_factor_doc_'):
                        ...
                        pass
                         ...
                         # ... (deletion logic) ...
                        pass
                # Add logging for deleted counts

                # 6. Create budget transaction linked to the Factor
                # 6. ایجاد تراکنش بودجه (فقط برای ذخیره نهایی)
                if save_final:
                ...
                #     allocation=budget_allocation_instance,
                #     transaction_type='CONSUMPTION',
                #     amount=total_items_amount, # Total amount consumed
                #     related_obj=self.object,    # Link transaction to the Factor
                #     created_by=self.request.user,
                #     description=f"ایجاد فاکتور {self.object.number}",
                #     transaction_id=f"TX-FACTOR-NEW-{self.object.id}-{timezone.now().timestamp()}"
                # )
                # logger.info(f"Budget transaction created for Factor pk={self.object.pk}, amount={total_items_amount}.")

        except ValidationError as ve:
             # Catch validation errors raised during model save/clean within atomic block
             ...
             # Add errors back to the forms if possible
             if hasattr(ve, 'error_dict'):
                 for field, errors in ve.error_dict.items():
                     if field == '__all__':
                         form.add_error(None, errors)
                     else:
                        # Try adding to main form or find appropriate item form
                        if field in form.fields:
                             form.add_error(field, errors)
                        else: # Try adding to item formset non-form errors
                             item_formset.add_error(None, f"{field}: {errors}") # Less ideal
             else:
                 form.add_error(None, str(ve)) # General error
             return self.render_to_response(self.get_context_data(form=form, formset=item_formset, document_form=document_form, tankhah_document_form=tankhah_document_form))

        except Exception as e:
            # Catch any other unexpected error during the atomic block
