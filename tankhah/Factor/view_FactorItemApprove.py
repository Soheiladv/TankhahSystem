...
class  ____FactorItemApproveView(PermissionBaseView, DetailView):
    model = Factor
    ...
    def send_notifications(self, entity, action, priority, description, recipients=None):
        ...
        for rule in rules:
            recipients = recipients or rule.recipients.all()
            for post in recipients:
                users = CustomUser.objects.filter(
                    userpost__post=post,
                    userpost__is_active=True,
                    userpost__post__organization=entity.tankhah.organization
                )
                for user in users:
                    notify.send(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not can_edit_approval(user, tankhah, tankhah.current_stage):
            messages.error(request,
                           _('شما دسترسی لازم برای ویرایش در این مرحله را ندارید یا سطح بالاتری اقدام کرده است.'))
            return redirect('factor_item_approve', pk=factor.pk)
        ...
        if 'change_stage' in request.POST:
            try:
                ...
                new_stage = WorkflowStage.objects.filter(order=new_stage_order, entity_type='FACTOR').first()
                if not new_stage:
                ...
                with transaction.atomic():
                    ...
                    tankhah.save(update_fields=['current_stage', 'status'])  # فقط فیلدهای لازم ذخیره شوند
        ...
        if request.POST.get('reject_factor'):
            with transaction.atomic():
                ...
                factor.rejected_reason = request.POST.get('rejected_reason', 'دلیل مشخص نشده')
                factor.save()
                ...
                ApprovalLog.objects.create(
                    ...
                    comment=f"رد کامل فاکتور: {factor.rejected_reason}",
                    post=user_post.post
                )
                self.send_notifications(
                    entity=factor,
                    action='REJECTED',
                    priority='HIGH',
                    description=f"فاکتور {factor.number} رد شد. لطفاً در تنخواه جدید ثبت کنید. دلیل: {factor.rejected_reason}",
                ...
                if all_factors_approved:
                    ...
                    if next_stage:
                        ...
                        if tankhah.current_stage.triggers_payment_order:
                            ...
                            tankhah.save(update_fields=['status'])
                            ApprovalLog.objects.create(
                                tankhah=tankhah,
                                factor=factor,
                                user=user,
            ...
            try:
                with transaction.atomic():
                    ...
                    for form in formset:
                        if form.cleaned_data:
                            ...
                            if status and status != 'NONE' and status != item.status:
                                ...
                                item.status = status
                                form.save()
                                logger.info(f"[ApproveView-POST] وضعیت آیتم {item.id} به {status} تغییر یافت.")
                    ...
                    if has_changes:
                        ...
                        if any_rejected:
                            factor.status = 'REJECTED'
                            factor.rejected_reason = 'یکی از آیتم‌ها رد شده است'
                            factor.save()
                            self.send_notifications(
                                entity=factor,
                                action='REJECTED',
                                priority='HIGH',
                                description=f"فاکتور {factor.number} به دلیل رد آیتم‌ها رد شد. لطفاً در تنخواه جدید ثبت کنید.",
                                recipients=[factor.created_by_post] if factor.created_by_post else []
                            )
                        elif all_approved:
                            ...
                            next_stage = WorkflowStage.objects.filter(
                                entity_type='FACTOR',
                                order__gt=factor.current_stage.order
                            ).order_by('order').first()
                            if next_stage and factor.current_stage.auto_advance:
                                factor.current_stage = next_stage
                                factor.save()
                                approving_posts = StageApprover.objects.filter(
                                    stage=next_stage,
                                    is_active=True
                            ...
                            elif factor.current_stage.is_final_stage and tankhah.current_stage.triggers_payment_order:
                                self.create_payment_order(factor, user)
                        else:
                ...
                logger.error(f"[ApproveView-POST] خطا در پردازش فرم‌ست: {e}", exc_info=True)
    ...
    def create_payment_order(self, factor, user):
        ...
        for post in approving_posts:
            ApprovalLog.objects.create(
                ...
                approver_post=Post.objects.get(pk=post)
            )
...
class FactorItemApproveView(PermissionBaseView, DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        logger.info(f"شروع dispatch در FactorItemApproveView برای کاربر: {request.user.username}")
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = _('تأیید ردیف‌های فاکتور') + f" - {factor.number}"
        context['tankhah'] = tankhah
        context['can_edit'] = user.is_hq or can_edit_approval(user, tankhah, tankhah.current_stage)
        context['can_change_stage'] = context['can_edit'] and tankhah.current_stage.order < user_level
        context['workflow_stages'] = WorkflowStage.objects.filter(order__lte=max_change_level).order_by('order')
        context['show_payment_number'] = tankhah.status == 'APPROVED' and not tankhah.payment_number
        all_items_approved = all(
        ...
        context['can_final_approve_tankhah'] = context['can_edit'] and all(
            f.status == 'APPROVED' for f in tankhah.factors.all())
        context['higher_approval_changed'] = ApprovalLog.objects.filter(
            tankhah=tankhah, post__level__lt=user_level,
            action__in=['APPROVE', 'REJECT', 'STAGE_CHANGE']
        ).exists()
        context['is_final_approved'] = ApprovalLog.objects.filter(
            tankhah=tankhah, action='STAGE_CHANGE', comment__contains='تأیید نهایی'
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(f"[ApproveView-POST] خطا در دریافت اطلاعات اولیه: {e}", exc_info=True)
            messages.error(request, _("خطا در پردازش درخواست."))
            return redirect('factor_item_approve', pk=self.kwargs.get('pk'))

        if not user_post and not user.is_hq:
            logger.error(f"کاربر {user.username} هیچ پست فعالی ندارد")
            messages.error(request, _("شما پست فعالی ندارید."))
            return redirect('factor_item_approve', pk=factor.pk)
        ...
        if tankhah.is_locked or tankhah.is_archived or factor.is_locked:
            ...
            return redirect('factor_item_approve', pk=factor.pk)
        if not user.is_hq and not can_edit_approval(user, tankhah, tankhah.current_stage):
            messages.error(request,
                           _('شما دسترسی لازم برای ویرایش در این مرحله را ندارید یا سطح بالاتری اقدام کرده است.'))
            return redirect('factor_item_approve', pk=factor.pk)
        ...
        if 'change_stage' in request.POST:
            try:
                new_stage_order = int(request.POST.get('new_stage_order'))
                if not user.is_hq and tankhah.current_stage.order >= user_post.post.level:
                    raise ValidationError(_('شما نمی‌توانید به مرحله برابر یا بالاتر تغییر دهید.'))
                if not user.is_hq and new_stage_order > user_post.post.max_change_level:
                    raise ValidationError(
                        _(f"سطح انتخابی ({new_stage_order}) بیشتر از حد مجاز شما ({user_post.post.max_change_level}) است."))
                ...
                logger.error(f"[ApproveView-POST] خطا در تغییر مرحله: {e}", exc_info=True)
                messages.error(request, str(e))
            except Exception as e:
        ...
        if request.POST.get('reject_factor'):
            try:
                with transaction.atomic():
                    ...
                    logger.info(
                        f"[ApproveView-POST] فاکتور {factor.pk} و {items_updated} آیتم آن رد شد توسط {user.username}")
                    ApprovalLog.objects.create(
                        tankhah=tankhah,
                        factor=factor,
                        user=user,
                ...
                logger.error(f"[ApproveView-POST] خطا در رد فاکتور: {e}", exc_info=True)
        ...
        if request.POST.get('final_approve'):
            try:
                with transaction.atomic():
                    ...
                    if all_factors_approved:
                        current_stage_order = tankhah.current_stage.order
                        next_stage = WorkflowStage.objects.filter(order__gt=current_stage_order).order_by(
                            'order').first()
                        if next_stage:
                            tankhah.current_stage = next_stage
                            tankhah.status = 'PENDING'
                ...
                logger.error(f"[ApproveView-POST] خطا در تأیید نهایی: {e}", exc_info=True)
                    ...
                    if has_changes:
                        ...
                        if any_rejected:
                            ...
                            messages.success(request,
                                             _('تمام ردیف‌های معتبر تأیید شدند. می‌توانید تأیید نهایی را انجام دهید.'))
                        else:
                            factor.status = 'PENDING'
                            messages.warning(request, _('لطفاً وضعیت تمام ردیف‌ها را مشخص کنید.'))

                        factor.save()
                        logger.info(f"[ApproveView-POST] وضعیت فاکتور {factor.id} به {factor.status} تغییر یافت.")
                        messages.success(request, _('تغییرات در وضعیت ردیف‌ها با موفقیت ثبت شد.'))
                    else:
                        messages.info(request, _('هیچ تغییری برای ثبت وجود نداشت.'))
...
#OK
class FactorItemApproveView_OK__OLD(PermissionBaseView, DetailView):
    model = Factor
    template_name = 'tankhah/factor_item_approve.html'
    permission_required = 'tankhah.FactorItem_approve'
    context_object_name = 'factor'
