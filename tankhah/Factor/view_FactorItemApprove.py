            ...
            with transaction.atomic():
                ...
                if all_factors_approved:
                    ...
                    if next_stage:
                        ...
                        tankhah.save()
                        ApprovalLog.objects.create(
                            tankhah=tankhah, user=user, action='STAGE_CHANGE', stage=next_stage,
                            comment=f"تایید نهایی و انتقال به {next_stage.name}", post=user_post.post if user_post else None
                        )
