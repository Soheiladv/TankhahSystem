...
from budgets.models import BudgetTransaction, ProjectBudgetAllocation, Tankhah, BudgetHistory, BudgetPeriod, \
    BudgetAllocation
...
class BudgetTransferForm(forms.Form):
    source_allocation = forms.ModelChoiceField(
        queryset=ProjectBudgetAllocation.objects.none(),
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        valid_project_allocations = ProjectBudgetAllocation.objects.filter(
            is_active=True,
            is_locked=False,  # قفل خود ProjectBudgetAllocation
    ...
    def clean(self):
        ...
        # اطمینان از وجود و صحت متد get_remaining_amount در مدل ProjectBudgetAllocation
        if not hasattr(source_pba, 'get_remaining_amount') or not callable(source_pba.get_remaining_amount):
             logger.error(f"Method 'get_remaining_amount' missing on ProjectBudgetAllocation {source_pba.pk}")
    ...
    #     # یا به فیلد is_locked که در مدل BudgetPeriod/BudgetAllocation/ProjectBudgetAllocation اضافه کردیم ارجاع دهید.
    #     if source_pba.is_locked or source_pba.budget_allocation.is_locked or source_pba.budget_allocation.budget_period.is_locked:
    ...
    #         raise ValidationError(_("جابجایی بودجه فقط بین تخصیص‌های یک دوره بودجه امکان‌پذیر است."))
    #
    #     # **مهم:** اطمینان از وجود و صحت متد get_remaining_amount در مدل ProjectBudgetAllocation
    #     if not hasattr(source_pba, 'get_remaining_amount') or not callable(source_pba.get_remaining_amount):
    #          logger.error(f"Method 'get_remaining_amount' not found or not callable on ProjectBudgetAllocation {source_pba.pk}")
    #          raise ValidationError(_("خطای سیستمی: امکان محاسبه باقیمانده مبدأ وجود ندارد."))
    #
    #     source_remaining = source_pba.get_remaining_amount()
    #     if amount > source_remaining:
    ...
    def execute_transfer(self):
        ...
        with transaction.atomic():
            ...
            # ۲. کاهش سهم ProjectBudgetAllocation مبدأ
            ...
            # ۳. ثبت تاریخچه برای مبدأ (ProjectBudgetAllocation)
            ...
            # ۵. افزایش سهم ProjectBudgetAllocation مقصد
            ...
            # ۶. ثبت تاریخچه برای مقصد (ProjectBudgetAllocation)
            BudgetHistory.objects.create(
...
class BudgetReturnForm(forms.Form):
    ...
    def __init__(self, *args, user=None, allocation=None, **kwargs):
        ...
        if allocation:
            ...
            self.fields['allocation'].queryset = ProjectBudgetAllocation.objects.filter(
                pk=allocation)
