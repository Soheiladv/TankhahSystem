...
class BudgetUserGuideView(PermissionBaseView, TemplateView):
    """
    راهنمای کاربر سیستم بودجه (بدون اطلاعات فنی)
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            # آمار ساده برای کاربر
            user_stats = {
                'total_periods': BudgetPeriod.objects.filter(is_active=True).count(),
                'total_allocations': BudgetAllocation.objects.filter(is_active=True).count(),
                'recent_transactions': BudgetTransaction.objects.count(),
            }
            
            context['user_stats'] = user_stats
            logger.info(f"User guide loaded with stats: {user_stats}")
            
        except Exception as e:
            logger.error(f"Error loading user guide stats: {str(e)}")
...
class BudgetAdminGuideView(PermissionBaseView, TemplateView):
    """
    راهنمای ادمین سیستم بودجه (با اطلاعات فنی)
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            logger.info(f"Admin guide loaded successfully: {context['stats']}")
            
        except Exception as e:
            logger.error(f"Error loading admin guide stats: {str(e)}")
            context['stats'] = None
            context['error'] = str(e)
