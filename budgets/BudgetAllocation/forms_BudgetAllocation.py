...
class BudgetAllocationForm(forms.ModelForm):
    ...
    def __init__(self, *args, user=None, budget_period=None, **kwargs):
        ...
        # Log incoming constructor data for diagnostics
        try:
            incoming_initial = kwargs.get('initial') or {}
            incoming_data = kwargs.get('data') or {}
            logger.info(
                "BudgetAllocationForm.__init__: initial_keys=%s data_keys=%s user=%s budget_period_kw=%s",
                list(incoming_initial.keys()) if hasattr(incoming_initial, 'keys') else type(incoming_initial),
                list(incoming_data.keys()) if hasattr(incoming_data, 'keys') else type(incoming_data),
                getattr(self.user, 'username', None),
                getattr(budget_period, 'id', None),
            )
        except Exception:
            pass
        ...
        logger.debug(
            "Form init resolved budget_period: %s",
            getattr(self.budget_period, 'id', None)
        )

        # در صورت ارسال POST و نبودن مقادیر فیلدهای پیشرفته، آن‌ها را از دوره بودجه پر کن
        try:
            if self.budget_period and hasattr(self, 'data') and self.data:
                mutable_data = self.data.copy()
                if not mutable_data.get('locked_percentage'):
                    mutable_data['locked_percentage'] = str(self.budget_period.locked_percentage or 0)
                if not mutable_data.get('warning_threshold'):
                    mutable_data['warning_threshold'] = str(self.budget_period.warning_threshold or 0)
                if not mutable_data.get('warning_action'):
                    mutable_data['warning_action'] = str(self.budget_period.warning_action or '')
                if not mutable_data.get('allocation_number'):
                    mutable_data['allocation_number'] = '0'
                self.data = mutable_data
        except Exception as _e:
            logger.debug(f"Failed to backfill advanced fields from budget_period: {_e}")

        # نمایش همه آیتم‌های فعال بدون اجبار به تطابق با دوره برای جلوگیری از افزونگی
        self.fields['budget_item'].queryset = BudgetItem.objects.filter(is_active=True).order_by('name')
        # برچسب گزینه خالی برای وضوح بیشتر
        self.fields['budget_item'].empty_label = _("— انتخاب ردیف بودجه —")
        try:
            logger.info(
                "BudgetAllocationForm: budget_item queryset count=%s (active only)",
                self.fields['budget_item'].queryset.count()
            )
        except Exception:
            pass
        ...
        try:
            logger.debug(
                "BudgetAllocationForm: budget_period field queryset ids=%s",
                list(self.fields['budget_period'].queryset.values_list('id', flat=True))
            )
        except Exception:
            pass
        ...
        if self.budget_period and not self.instance.pk:
            ...
            logger.debug(
                "Initials from budget_period(id=%s): locked=%s warn_th=%s warn_action=%s",
                self.budget_period.id,
                self.initial.get('locked_percentage'),
                self.initial.get('warning_threshold'),
                self.initial.get('warning_action'),
            )
    ...
    def clean(self):
        ...
        # عدم اجبار به تطابق ردیف بودجه با دوره انتخاب‌شده (برای جلوگیری از افزونگی)
        # صرفاً اطمینان از انتخاب شدن ردیف بودجه انجام شده است؛ تطابق دوره‌ای حذف شد
        ...
        # Log core cleaned values for diagnostics
        try:
            logger.info(
                "Form.clean: bp=%s org=%s item=%s proj=%s amount=%s type=%s date=%s",
                getattr(budget_period, 'id', None),
                getattr(organization, 'id', None),
                getattr(budget_item, 'id', None),
        ...
        # اعتبارسنجی مدل فقط در صورت داشتن دوره بودجه معتبر
        if budget_period:
            try:
                ...
                self.add_error(None, _(
                    f"خطا در اعتبارسنجی تخصیص: {str(e)}"
                ))
