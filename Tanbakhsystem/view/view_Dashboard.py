...
class  old_DashboardView(LoginRequiredMixin, View):
    ...
    def get_project_total_budget(self, project, filters=None):
        try:
            ...
            logger.info(
                f"get_project_total_budget: project={project.id}, name={project.name}, "
                f"total={total}, filters={filters}"
            )
            return total
    ...
    def get_project_used_budget(self, project, filters=None):
        try:
            ...
            logger.info(
                f"get_project_used_budget: project={project.id}, name={project.name}, "
                f"total={total}, filters={filters}"
            )
            return total
        except Exception as e:
    ...
    def get_project_remaining_budget(self, project, filters=None):
        try:
            ...
            logger.info(
                f"get_project_remaining_budget: project={project.id}, name={project.name}, "
                f"remaining={remaining}, filters={filters}"
            )
            return remaining
        except Exception as e:
    ...
    def get_context_data(self, request):
        ...
        current_gregorian_month = now.month
        ...
        context['dashboard_links'] = {
            ...
            'قوانین سیستم (رول‌های دسترسی)': [
                ...
                {'name': _('قوانین سیستم (رول‌های دسترسی) فرم جدید'), 'url': 'post_access_rule_assign', 'icon': 'fas fa-history'},
            ],
            'تاریخچه پست‌ها': [
        ...
        if context['can_view_budget_stats']:
            try:
                ...
                total_allocated = active_budget_periods.aggregate(
                ...
                logger.info(
                    ...
                    f"active_budget_periods_count={active_budget_periods.count()}"
                )
                ...
                for i in range(5, -1, -1):
                    ...
                    if month_label not in seen_months:
                        ...
                        logger.debug(
                            f"Monthly budget stats: month={month_label}, allocated={monthly_allocated}, "
                            f"consumed={monthly_consumed}"
                        )
                ...
                logger.info(
                    f"Budget vs actual: months={budget_vs_actual_labels}, "
                    f"allocated={budget_vs_actual_allocated_data}, consumed={budget_vs_actual_consumed_data}"
                )
            except Exception as e:
        ...
        logger.info(f"Context prepared: keys={context.keys()}")
        return context
    ...
    def _get_tankhah_stats(self):
        ...
        try:
            ...
            stats.update({
                'active_tankhah_count': Tankhah.objects.filter(
                    status__in=['PENDING', 'APPROVED', 'SENT_TO_HQ', 'HQ_OPS_PENDING', 'HQ_FIN_PENDING']
                ).count(),
            ...
            for i in range(5, -1, -1):
                ...
                end_date = (jdatetime.date(year, month,
                                           jdatetime.j_days_in_month[month - 1]).togregorian() + timedelta(days=1))

                monthly_factors.append({
                    'label': f"{self.get_jalali_month_name(month)} {year}",
            ...
            stats['monthly_factors_data'] = {
                'labels': [m['label'] for m in monthly_factors],
                'values': [m['value'] for m in monthly_factors]
            }
            ...
            stats['quarterly_factors_data'] = {
                'labels': [q['label'] for q in quarterly_factors],
                'values': [q['value'] for q in quarterly_factors]
            }

        except Exception as e:
            logger.error(f"Error in tankhah stats: {str(e)}", exc_info=True)
...
class  DashboardView(LoginRequiredMixin, View):
    ...
    def get_context_data(self, request):
        ...
        context['dashboard_links'] = {
            'روند تنخواه': [
                {'name': _('روند تنweweخواه'), 'url': 'simple_chart', 'icon': 'fas fa-link'},
                {'name': _('روند تنخواه'), 'url': 'dashboard_flows', 'icon': 'fas fa-link'},
                {'name': _('BI گزارشات'), 'url': 'financialDashboardView', 'icon': 'fas fa-chart-bar'},
                {'name': _('گزارش جزئیات تنخواه'), 'url': 'tankhah_detail', 'icon': 'fas fa-chart-bar'},
                {'name': _('گزارش لحظه‌ای از بودجه‌بندی'), 'url': 'budgetrealtimeReportView', 'icon': 'fas fa-chart-bar'},
